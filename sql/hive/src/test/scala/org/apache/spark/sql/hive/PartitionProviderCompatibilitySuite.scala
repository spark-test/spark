...
class PartitionProviderCompatibilitySuite
  ...
  private def setupPartitionedDatasourceTable(tableName: String, dir: File): Unit = {
    ...
    spark.sql(s"""
      ...
      |using parquet
      |options (path "${dir.toURI.toString}")
      ...
      withTable("test") {
        ...
        withTempDir { dir =>
          ...
          spark.sql(s"""
            ...
            |set location '${dir.toURI.toString}'""".stripMargin)
  ...
  private def testCustomLocations(testFn: => Unit): Unit = {
    ...
    try {
      spark.sql(s"""
        ...
        |using parquet
        |options (path "${base.toURI.toString}")
        |partitioned by (P1, P2)""".stripMargin)
      spark.sql(s"alter table test add partition (P1=0, P2=0) location '${a.toURI.toString}'")
      spark.sql(s"alter table test add partition (P1=0, P2=1) location '${b.toURI.toString}'")
      spark.sql(s"alter table test add partition (P1=1, P2=0) location '${c.toURI.toString}'")
